...
class VaspInterface():
    ...
    def __get_first_word_last_line(self, calc_dir: str):
        return subprocess.run(["tail", "-n", "1", calc_dir + "/OUTCAR"], capture_output = True, text = True)
    ...
    def __determine_continuation(self, log_dictionary: dict, step_name: str) -> bool:
        # make this list separate from the dictionary so there's no runtime error 
    ...
    def __log_calculation_status(self, calc_dir: Path):
        outcar_path = Path(calc_dir) / "OUTCAR"
        
        # Check if the OUTCAR was created
        if outcar_path in calc_dir.iterdir():
    ...
    # TODO: Break down this function into different functions, especially ones I
    # repeatedly use
    def initialize(self):
        ...
        # check if its a directory through an exception
        #try:
        # initialize everything besides POSCARs, which will change
        # TODO: ask if they want to change the POSCAR if they believe the potpawpbte is valid
        # either way, don't continue if it cant get a valid atom from the potpawpbte
        ...
        #    print("Probably not a directory, whoops!",err)
        ...
        # ask for max time in minutes for 
        ...
        # set up the directory. this would be given by the user
        ...
        # this part can probably be split into either processes or threads
        for poscar_path in original_poscar_dir:
...
class VaspInit():
    def __init__(self):
        ...
        self.poscar_info = None
    
    # copy constructor
    def copy_init(self, vasp_init):
        self.validator = vasp_init.validator
    ...
    def set_poscar(self, poscar_path: str) -> bool:
        self.validator = FileValidation(poscar_path)

        self.poscar_file = self.validator.valid_file()
        if self.poscar_file:
            try:
    ...
    def set_potpawpbte(self, potpawpbte_path: str) -> bool:
        self.validator = FileValidation(potpawpbte_path)

        self.potpawpbte_dir = self.validator.valid_directory()
    ...
    def set_hdf5(self, hdf5_lib_path: str):
        self.validator = FileValidation(hdf5_lib_path)
        self.validator.change_path(hdf5_lib_path)

        self.hdf5_lib_dir = self.validator.valid_directory()
    ...
    def set_incar(self, incar_path: str):
        ...
        self.validator = FileValidation(incar_path)
        self.validator.change_path(incar_path)
        
        self.incar_file = self.validator.valid_file()
        if self.incar_file:
            print("Valid INCAR!\n")
            return True
        else:
    ...
    def set_vasp(self, vasp_bin_path: str):
        self.validator.change_path(vasp_bin_path)

        self.vasp_bin_file = self.validator.valid_file()
    ...
    def initialize(self):
        ...
        if init_dict != {}:
            ...
            self.validator = FileValidation(init_dict["poscar"])
            self.poscar_file = self.validator.valid_file()

            self.validator.change_path(init_dict["incar"])
            self.incar_file = self.validator.valid_file()

            self.validator.change_path(init_dict["potpawpbte"])
            self.potpawpbte_dir = self.validator.valid_directory()

            self.validator.change_path(init_dict["hdf5-lib"])
            self.hdf5_lib_dir = self.validator.valid_directory()

            self.validator.change_path(init_dict["vasp_std"])
            self.vasp_bin_file = self.validator.valid_file()
